...
type FlightDataWriter interface {
...
func NewWriter(w FlightDataWriter, opts ...Option) *Writer {
	cfg := newConfig(opts...)
	return &Writer{
		w:      w,
		mem:    cfg.alloc,
...
func (w *Writer) writePayload(data *payload) (err error) {
...
func (w *recordEncoder) visit(p *payload, arr array.Interface) error {
	...
	case arrow.FixedWidthDataType:
		...
		case needTruncate(int64(data.Offset()), values, minLength):
			...
			n := minI64(bitutil.CeilByte64(arrLen*typeWidth), int64(data.Len())-offset)
			data = array.NewSliceData(data, offset, offset+n)
			defer data.Release()
			values = data.Buffers()[1]
		}
		if values != nil {
	...
	case *arrow.BinaryType:
		...
		case needTruncate(int64(data.Offset()), values, totalDataBytes):
			...
			var (
				...
				n   = minI64(paddedLength(totalDataBytes, kArrowAlignment), int64(data.Len())-beg)
			)
			data = array.NewSliceData(data, beg, beg+n)
			defer data.Release()
			values = data.Buffers()[2]
		default:
			if values != nil {
