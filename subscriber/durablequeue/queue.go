...
func (l *Queue) Append(b []byte) error {
	...
	bytesWritten, err := l.tail.append(b)
...
type segment struct {
	...
	currentSize int64        // Size of the current block.
	...
	buf         bytes.Buffer // Buffer for append() to reduce syscalls.
...
func (l *segment) open() error {
	...
	if l.currentSize > l.size-footerSize-l.pos || l.currentSize < 0 {
...
func (l *segment) append(b []byte) (int64, error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	...
	if _, err := l.buf.Write(buf[:]); err != nil {
		return 0, err
	}
	...
	if err := l.writeBytes(l.buf.Bytes()); err != nil {
		return 0, err
	}
...
func (l *segment) advance() error {
	return l.advanceTo(l.pos + l.currentSize + 8)
}
