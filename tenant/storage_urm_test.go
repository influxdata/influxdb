...
func TestURM(t *testing.T) {
	...
	st := []struct {
		...
		},
		{
			...
			results: func(t *testing.T, store *tenant.Store, tx kv.Tx) {
				urms, err := store.ListURMs(context.Background(), tx, influxdb.UserResourceMappingFilter{UserID: influxdb.ID(1)}, influxdb.FindOptions{Offset: 2, Limit: 10})
				if err != nil {
					t.Fatal(err)
				}
