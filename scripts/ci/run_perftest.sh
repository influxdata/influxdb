...
cleanup() {
  ...
  if [ "${CIRCLE_TEARDOWN}" = true ]; then
    curl --request POST \
      ...
      --data "{\"branch\":\"${INFLUXDB_VERSION}\", \"parameters\":{\"aws-teardown\": true, \"aws-teardown-branch\":\"${INFLUXDB_VERSION}\", \"aws-teardown-sha\":\"${TEST_COMMIT}\", \"aws-teardown-datestring\":\"${CIRCLE_TEARDOWN_DATESTRING}\", \"aws-teardown-query-format\":\"${TEST_FORMAT}\"}}"
  fi
}
