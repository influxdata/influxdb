...
`CatalogRecord` is generated by `#[catalog_record]` (`oss/influxdb3_catalog_macros`), which also derives the encoding, registers the type, and enforces the frozen layout against a declared field-shape fingerprint. `RecordApply` is the hand-written half — see [Adding a new record / feature](#adding-a-new-record--feature).
...
4. **persist** — the file is written to object store with an optimistic-concurrency conditional write, under the `write_permit`. See [Optimistic concurrency](#optimistic-concurrency).
...
format/
  ...
  registry.rs                       `CatalogRecord`/`RecordApply` traits + `inventory` registry (BTreeMap<RecordId, _>)
  ...
  records/                          the frozen record structs (database, table, cache, …)
                                    declared with `#[catalog_record]` from `oss/influxdb3_catalog_macros`
...
**1. Define the record struct and its `RecordApply` impl** (`src/format/records/<resource>.rs`). Use the next sequential ID in the correct partition; feature-gated unless it genuinely meets the `UPGRADE_SAFE` criteria.

```rust
#[catalog_record(id = record_ids::SET_DATABASE_DESCRIPTION, shape = 0x00000000)]
pub struct SetDatabaseDescription { pub db_id: u32, pub description: String }

impl RecordApply for SetDatabaseDescription {
...
`#[catalog_record]` derives the encoding, implements `CatalogRecord` (`NAME` is the type name, `FLAGS` defaults to `RecordFlags::none()`), and registers the type with `inventory` so the registry and the derived feature level pick it up. Add `#[derive(Copy)]` below the attribute where the body is `Copy`.

**2. Fill in the shape fingerprint.** Build with the `0x00000000` placeholder; the compile error names the actual value to paste back. The fingerprint covers the field types in order, so a later field add, removal, reorder, or retype fails the build — including on fields no round-trip fixture exercises. Field renames are free, because bitcode does not encode names.

Fields are also checked against an allowlist: primitives, `String`, tuples, arrays, `Option`/`Vec` of those, and wire types declared in `records::types`. A qualified path such as `influxdb3_authz::role::Permission` is rejected — the catalog does not own that type's layout, so it cannot go on the wire. Declare a wire type alongside the records and convert in `apply`.
...
```rust
#[catalog_record(
    id = record_ids::SET_NODE_LICENSE_DIGEST,
    shape = 0x1a2b3c4d,
    flags = RecordFlags::upgrade_safe(),   // gate-exempt
)]
pub struct SetNodeLicenseDigest { /* … */ }
```
...
- **_NEVER_** modify or reorder a shipped record or its ID; only append. Deprecate by removing the producer code while keeping the struct, its `#[catalog_record]` attribute, and its `RecordApply` impl.
