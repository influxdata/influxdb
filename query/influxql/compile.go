...
func newCompiledStatement(config Config) *compiledStatement {
...
func compile(stmt *influxql.SelectStatement, config Config) (*compiledStatement, error) {
	c := newCompiledStatement(config)
	c.stmt = stmt.Clone()
...
func (c *compiledStatement) preprocess(stmt *influxql.SelectStatement) error {
	...
	valuer := influxql.NowValuer{Now: c.Config.Now, Location: stmt.Location}
...
func (c *compiledStatement) subquery(stmt *influxql.SelectStatement) error {
	subquery := newCompiledStatement(c.Config)
	...
	valuer := influxql.MultiValuer(
		&influxql.NowValuer{Now: c.Config.Now, Location: stmt.Location},
		&MathValuer{},
	)
