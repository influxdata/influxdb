...
func TestCompile_Failures(t *testing.T) {
	for _, tt := range []struct {
		...
		{s: `SELECT exp_moving_average(field1, 2), field1 FROM myseries`, err: `mixing aggregate and non-aggregate queries is not supported`},
		{s: `SELECT exp_moving_average(field1, 1), field1 FROM myseries`, err: `exp_moving_average window must be greater than 1, got 1`},
		{s: `SELECT exp_moving_average(field1, 0), field1 FROM myseries`, err: `exp_moving_average window must be greater than 1, got 0`},
		{s: `SELECT exp_moving_average(field1, -1), field1 FROM myseries`, err: `exp_moving_average window must be greater than 1, got -1`},
		{s: `SELECT exp_moving_average(field1, 2.0), field1 FROM myseries`, err: `second argument for exp_moving_average must be an integer, got *influxql.NumberLiteral`},
		{s: `SELECT exp_moving_average() from myseries`, err: `invalid number of arguments for exp_moving_average, expected 2, got 0`},
		{s: `SELECT exp_moving_average(value) FROM myseries`, err: `invalid number of arguments for exp_moving_average, expected 2, got 1`},
		{s: `SELECT exp_moving_average(value, 2) FROM myseries group by time(1h)`, err: `aggregate function required inside the call to exp_moving_average`},
		{s: `SELECT exp_moving_average(top(value), 2) FROM myseries where time < now() and time > now() - 1d group by time(1h)`, err: `invalid number of arguments for top, expected at least 2, got 1`},
		{s: `SELECT exp_moving_average(bottom(value), 2) FROM myseries where time < now() and time > now() - 1d group by time(1h)`, err: `invalid number of arguments for bottom, expected at least 2, got 1`},
		{s: `SELECT exp_moving_average(max(), 2) FROM myseries where time < now() and time > now() - 1d group by time(1h)`, err: `invalid number of arguments for max, expected 1, got 0`},
		{s: `SELECT exp_moving_average(percentile(value), 2) FROM myseries where time < now() and time > now() - 1d group by time(1h)`, err: `invalid number of arguments for percentile, expected 2, got 1`},
		{s: `SELECT exp_moving_average(mean(value), 2) FROM myseries where time < now() and time > now() - 1d`, err: `exp_moving_average aggregate requires a GROUP BY interval`},
