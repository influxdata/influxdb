...
func TestHandler_Nodes(t *testing.T) {
	srvr := OpenServer(NewMessagingClient())
	srvr.CreateNode(MustParseURL("http://localhost:1000"))
	srvr.CreateNode(MustParseURL("http://localhost:2000"))
	srvr.CreateNode(MustParseURL("http://localhost:3000"))
	s := NewHTTPServer(srvr)
	defer s.Close()

	status, body := MustHTTP("GET", s.URL+`/nodes`, "")
	if status != http.StatusOK {
		t.Fatalf("unexpected status: %d", status)
...
func TestHandler_CreateNode(t *testing.T) {
	...
	status, body := MustHTTP("POST", s.URL+`/nodes`, `{"url":"http://localhost:1000"}`)
	if status != http.StatusCreated {
		t.Fatalf("unexpected status: %d", status)
...
func TestHandler_CreateNode_BadRequest(t *testing.T) {
	srvr := OpenServer(NewMessagingClient())
	s := NewHTTPServer(srvr)
	defer s.Close()

	status, body := MustHTTP("POST", s.URL+`/nodes`, `{"name":`)
	if status != http.StatusBadRequest {
		t.Fatalf("unexpected status: %d", status)
...
func TestHandler_CreateNode_InternalServerError(t *testing.T) {
	srvr := OpenServer(NewMessagingClient())
	s := NewHTTPServer(srvr)
	defer s.Close()

	status, body := MustHTTP("POST", s.URL+`/nodes`, `{"url":""}`)
	if status != http.StatusInternalServerError {
		t.Fatalf("unexpected status: %d", status, body)
	} else if body != `node url required` {
		t.Fatalf("unexpected body: %s", body)
	}
}
...
func TestHandler_DeleteNode(t *testing.T) {
	...
	status, body := MustHTTP("DELETE", s.URL+`/nodes/1`, "")
	if status != http.StatusNoContent {
		t.Fatalf("unexpected status: %d", status)
	} else if body != `` {
		t.Fatalf("unexpected body: %s", body)
...
func TestHandler_DeleteUser_NodeNotFound(t *testing.T) {
	srvr := OpenServer(NewMessagingClient())
	s := NewHTTPServer(srvr)
	defer s.Close()

	status, body := MustHTTP("DELETE", s.URL+`/nodes/10000`, "")
	if status != http.StatusNotFound {
		t.Fatalf("unexpected status: %d", status)
	} else if body != `node not found` {
		t.Fatalf("unexpected body: %s", body)
	}
}
