...
use data_types::{ChunkId, NamespaceId, PartitionHashId, PartitionId, TransitionPartitionId};
...
impl IngesterConnectionImpl {
    ...
    fn by_addrs_with_flight_client(
        ingester_addresses: Vec<Arc<str>>,
        flight_client: Arc<dyn IngesterFlightClient>,
        catalog_cache: Arc<CatalogCache>,
        backoff_config: BackoffConfig,
...
impl IngesterStreamDecoder {
    ...
    fn flush_chunk(&mut self) -> Result<()> {
        if let Some((schema, batches)) = self.current_chunk.take() {
            ...
            self.current_partition = Some(try_add_chunk(
                current_partition,
                ChunkId::new(),
                schema,
                batches,
...
pub(crate) fn try_add_chunk(
    partition: IngesterPartition,
    chunk_id: ChunkId,
    expected_schema: Schema,
    batches: Vec<RecordBatch>,
) -> Result<IngesterPartition> {
...
mod tests {
    ...
    fn test_ingester_partition_type_cast() {
        ...
        for case in cases {
            ...
            let ingester_partition = try_add_chunk(
                ingester_partition,
                ChunkId::new(),
