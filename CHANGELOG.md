...
-	[#9397](https://github.com/influxdata/influxdb/pull/9397): Fix stddev() call to report itself as always returning a float.
-	[#9492](https://github.com/influxdata/influxdb/pull/9492): Evaluate a true boolean literal when calculating tag sets.
-	[#9541](https://github.com/influxdata/influxdb/pull/9541): Allow time variable to be case insensitive again.
...
There are breaking changes in this release: - The filesystem hierarchy for packages has been changed, namely: - Binaries are now located in `/usr/bin` (previously `/opt/influxdb`) - Configuration files are now located in `/etc/influxdb` (previously `/etc/opt/influxdb`) - Data directories are now located in `/var/lib/influxdb` (previously `/var/opt/influxdb`) - Scripts are now located in `/usr/lib/influxdb/scripts` (previously `/opt/influxdb`\)
