...
RUN apt update \
    && apt install --yes binutils build-essential pkg-config libssl-dev clang lld git protobuf-compiler python3 python3-dev \
...
RUN apt update \
    && apt install --yes ca-certificates gettext-base libssl3 python3 python3-dev --no-install-recommends \
    && rm -rf /var/lib/{apt,dpkg,cache,log} \
