...
func TestPrepare_MaxTimeRange(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			shardMapper := ShardMapper{
				MapShardsFn: func(_ context.Context, _ influxql.Sources, _ influxql.TimeRange) query.ShardGroup {
					return &ShardGroup{
			...
			_, err = c.Prepare(context.Background(), &shardMapper, query.SelectOptions{MaxTimeRange: tt.maxTimeRange, MaxBucketsN: tt.maxBucketsN})
			if tt.wantErr {
				require.Error(t, err)
