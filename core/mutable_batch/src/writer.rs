...
use std::{fmt::Debug, iter::repeat_n, ops::Range};
...
/// truncated to the original number of rows
#[derive(Debug)]
    ...
    T: ColumnInsertValidator,
{
        ...
        V: Copy + IsNan + PartialOrd + Default + Debug + ContainedInVecInColumn,
    {
        ...
        let col = self.column_mut(name, V::INFLUX_COL_TY)?;
        ...
        V: Copy + IsNan + PartialOrd + ContainedInVecInColumn,
    {
        ...
        let col = self.column_mut(name, col_ty)?;
        ...
        I: Iterator<Item = f64>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Field(InfluxFieldType::Float))?;
        ...
        I: Iterator<Item = i64>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Field(InfluxFieldType::Integer))?;
        ...
        I: Iterator<Item = u64>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Field(InfluxFieldType::UInteger))?;
        ...
        I: Iterator<Item = bool>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Field(InfluxFieldType::Boolean))?;
        ...
        I: Iterator<Item = &'s str>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Field(InfluxFieldType::String))?;
        ...
        I: Iterator<Item = &'s str>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Tag)?;
        ...
        I: Iterator<Item = i64>,
    {
        ...
        let col = self.column_mut(name, InfluxColumnType::Timestamp)?;
    ...
    pub(crate) fn write_batch(&mut self, src: &MutableBatch) -> Result<()> {
        ...
        for (src_col_name, src_col_idx) in &src.column_names {
            ...
            let dst_col = self.column_mut(src_col_name, src_col.influx_type)?;

            match (&mut dst_col.data, &src_col.data) {
                (ColumnData::F64(dst_data), ColumnData::F64(src_data)) => {
                    dst_data.extend_from_slice(src_data);
                }
                (ColumnData::I64(dst_data), ColumnData::I64(src_data)) => {
                    dst_data.extend_from_slice(src_data);
                }
                (ColumnData::U64(dst_data), ColumnData::U64(src_data)) => {
                    dst_data.extend_from_slice(src_data);
                }
                (ColumnData::Bool(dst_data), ColumnData::Bool(src_data)) => {
                    dst_data.extend_from(src_data);
                }
                (ColumnData::String(dst_data), ColumnData::String(src_data)) => {
                    dst_data.extend_from(src_data);
                }
                (ColumnData::Tag(dst_data, dst_dict), ColumnData::Tag(src_data, src_dict)) => {
    ...
    pub(crate) fn write_batch_ranges(
        ...
        for (src_col_name, src_col_idx) in &src.column_names {
            ...
            match (&mut dst_col.data, &src_col.data) {
                (ColumnData::F64(dst_data), ColumnData::F64(src_data)) => {
                    write_slice(to_insert, ranges, src_data, dst_data)
                }
                (ColumnData::I64(dst_data), ColumnData::I64(src_data)) => {
                    write_slice(to_insert, ranges, src_data, dst_data)
                }
    ...
    fn column_mut(&mut self, name: &str, influx_type: InfluxColumnType) -> Result<&mut Column> {
    ...
    /// Commits the writes performed on this [`Writer`]. This will pad any unwritten columns with
    /// nulls
...
fn write_slice<T>(to_insert: usize, ranges: &[Range<usize>], src_data: &[T], dst_data: &mut Vec<T>)
where
    ...
    fn drop(&mut self) {
        if !self.success {
            ...
            for col in &mut self.batch.columns {
                ...
                match &mut col.data {
                    ColumnData::F64(col_data) => col_data.truncate(initial_rows),
                    ColumnData::I64(col_data) => col_data.truncate(initial_rows),
                    ColumnData::U64(col_data) => col_data.truncate(initial_rows),
                    ColumnData::String(col_data) => col_data.truncate(initial_rows),
                    ColumnData::Bool(col_data) => col_data.truncate(initial_rows),
                    ColumnData::Tag(col_data, dict) => {
