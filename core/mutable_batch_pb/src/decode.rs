...
pub fn write_table_batch(batch: &mut MutableBatch, table_batch: &TableBatch) -> Result<()> {
    ...
    let mut found_time = columns.first().is_some_and(|f| *f == TIME_COLUMN_NAME);

    // it would be nice to use `array_windows` here but alas, that's nightly-only
    for two_names in columns.windows(2) {
        let first = two_names[0];
        let second = two_names[1];
        ensure!(
            first != two_names[1],
            DuplicateColumnNameSnafu { column: first }
        );

        found_time |= second == TIME_COLUMN_NAME;
    }

    ensure!(found_time, MissingTimeSnafu);
    ...
    for column in &table_batch.columns {
        ...
        match influx_type {
            InfluxColumnType::Field(InfluxFieldType::Float) => match valid_mask {
                None => writer.write_f64s_from_slice(&column.column_name, &values.f64_values),
                Some(mask) => writer.write_f64s_from_slice_with_mask(
                    &column.column_name,
                    mask,
                    &values.f64_values,
                ),
            },
            InfluxColumnType::Field(InfluxFieldType::Integer) => match valid_mask {
                None => writer.write_i64s_from_slice(&column.column_name, &values.i64_values),
                Some(mask) => writer.write_i64s_from_slice_with_mask(
                    &column.column_name,
                    mask,
                    &values.i64_values,
                ),
            },
            InfluxColumnType::Field(InfluxFieldType::UInteger) => match valid_mask {
                None => writer.write_u64s_from_slice(&column.column_name, &values.u64_values),
                Some(mask) => writer.write_u64s_from_slice_with_mask(
                    &column.column_name,
                    mask,
                    &values.u64_values,
                ),
...
fn packed_strings_iter(strings: &PackedStrings) -> impl ExactSizeIterator<Item = &str> + '_ {
...
/// Try to get the type of the value stored in this column.
///
/// # Errors
///
/// This will return an error if:
/// 1. There are no values in this column
/// 2. The stored value for the semantic type doesn't correspond to any known value
/// 3. The type of the stored values doesn't match what we expect to see based on the semantic type
pub fn pb_column_type(col: &PbColumn) -> Result<InfluxColumnType> {
