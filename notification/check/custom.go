...
// UI
// Verify -> dry run of query which surfaces flux errors
// Valid
// find task option if not exists error
//		find task.every if not exists error,
//		find task.offset if not exists error,
//		check that task.name matches check.name
