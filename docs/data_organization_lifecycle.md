...
To trigger chunks moving from one stage to another, IOx creates some events. Note that IOx keeps chunks inside their (virtual) partition as defined in the previous Section. Hence, all chunks described in this section are in the context of one partition. In this context, there is always at most one `O-MUB` at a time.
...
* **Delete**: is an event issued by users (through gRPC API) to delete some data. When a delete is issued, All affected chunks are frozen its delete predicates are attached into all available chunks. So in this Data LifeCycle context, only O-MUBs are affected by delete events.
* **Persist**: is an event to compact eligible `O-MUB`, `F-MUBs`, and `RUBs` into 2 chunks, an `OS` of time window t1-t2 and a `RUB` of time window t2-t3, in which t1 and t3 are the start and end of those chunks time respectively, and t2 is a time defined by the `Data LifeCycle Policy`. 
...
The `Compact`, `Persist` and `Compact OS` can only compact/persist contiguous chunks which are chunks contain time-window next to each other. They also eliminate permanently duplicated and deleted data. When a new chunk is created, the compacted/persisted ones are dropped.

### Data LifeCycle Example
Figure 3 illustrates an example of IOx Data LifeCycle with 8 events happening at time T1 to time T8. Let us try to go over each event. Note that the number (1, 2, 3, ...) in each chunk name is just for us to identify them easily. They are not the real UUID identifiers of the chunks.

* **Before T1**: There is only `O-MUB 1` in the system for this partition. Ingesting data will go in that partition and query will also read data from it if applicable.
...
* **At T2**: a `Delete` is issued that freezes `O-MUB 2` to `F-MUB 2`. Since new data keeps coming, `O-MUB 3` is created for this new data.
* **At T3**: a `Compact` is requested on `RUB 1`, `F-MUB 2`, and `O-MUB 2`. As always, the `O-MUB 3` must be first frozen to `F-MUB 3` and then compacted with `RUB 1` and `F-MUB 2` to produce `RUB 2`, while `O-MUB 3` is created to accept ingesting data.
* **At T4**: a `Delete` triggers `O-MUB 3` frozen to `F-MUB 3` and `O-MUB 4` created.
* **At T5**: a `Compact` on `F-MUB 2` and `O-MUB 4` to produce `RUB 3` and triggers the creation of `O-MUB 5`.
* **At T6**: a `Persist` is issued on `RUB 2`, `RUB 3` and `O-MUB 5` to produce `OS 1` and `RUB 3`, and triggers the creation of `O-MUB 6`.
* **At T7**: a `Persist` is issued on `RUB 4` and `O-MUB 6` to produce `OS 2` and `RUB 5`, and triggers the creation of `O-MUB 7`.
