 ...
 * Call constrain
 */
public class Pconstrain extends NamedWarpScriptFunction implements WarpScriptStackFunction {
  
  public Pconstrain(String name) {
    super(name);
  }
  ...
  public Object apply(WarpScriptStack stack) throws WarpScriptException {
    ...
    boolean isint = params.get(1) instanceof Long;
    
    if (isint) {
      stack.push((long) PApplet.constrain(
          ((Number) params.get(1)).intValue(),
          ((Number) params.get(2)).intValue(),
          ((Number) params.get(3)).intValue()
          ));
    } else {
      stack.push((double) PApplet.constrain(
