 ...
 */
public class FILLPREVIOUS extends NamedWarpScriptFunction implements WarpScriptStackFunction {
  
  public FILLPREVIOUS(String name) {
    super(name);
  }
  ...
  public Object apply(WarpScriptStack stack) throws WarpScriptException {
    ...
    if (top instanceof List) {
      ...
      for (GeoTimeSerie gts: series) {
        result.add(GTSHelper.fillprevious(gts));
      }
      stack.push(result);
    } else if (top instanceof GeoTimeSerie) {
      stack.push(GTSHelper.fillprevious((GeoTimeSerie) top));
    } else {
      throw new WarpScriptException(getName() + " expects a LIST or a Geo Time Series™ as input.");
    }
