...
This script uses the `golang:1.7.5` image to build a fully static binary of `influxd` and then adds it to a minimal `scratch` image.

To build the image using a different version of go:

```
$ GO_VER=1.7.5 ./build-docker.sh
```
