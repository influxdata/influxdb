...
Likewise for /enterprise-git, currently the only other private repository.
"
}
...
if [ -d /enterprise-git ]; then
  git clone /enterprise-git "$IPATH/enterprise"
else
  # Add GitHub host key to known hosts, non-interactively.
  ssh-keyscan github.com >> ~/.ssh/known_hosts

  git clone git@github.com:influxdata/enterprise.git
fi
cd influxdb
git checkout "$SHA"
gdm restore
...
for cmd in \
  ...
  influxdb/cmd/influxd-ctl \
  influxdb/cmd/influxd-meta \
  ; do
printf 'package main
...
func init() {
	...
	branch = "%s"
	commit = "%s"
}' "$VERSION" "$BRANCH" "$SHA" > "./$cmd/version.generated.go"
done
