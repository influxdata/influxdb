...
func TestQueueScanRace(t *testing.T) {
	const numWrites = 100
	const writeSize = 270000
	q, dir := newTestQueue(t, withMaxSize(1024*1024*1024))
	defer os.RemoveAll(dir)
	reads, writes := 0, 0
	wg := sync.WaitGroup{}
	wg.Add(2)
	...
	go func() {
		...
		for {
			...
			for scan.Next() {
				// ensure bytes come out in the right order
				b := scan.Bytes()
				assert.Equal(t, writeSize, len(b))
				assert.Equal(t, byte(reads), b[0])
				reads++
			}
			_, err = scan.Advance()
			require.NoError(t, err)
	...
	wg.Wait()
	assert.Equal(t, numWrites, writes)
	assert.Equal(t, numWrites, reads)
}
...
func TestQueue_AdvanceSingleCorruptSegment(t *testing.T) {
	...
	mustScan := func(fn func(Scanner) int64, advanceErr string) {
		t.Helper()
		scan, err := q.NewScanner()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		...
		if advanceErr == "" && err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if advanceErr != "" {
			assert.EqualError(t, err, advanceErr)
		}
	...
	// at this point, the segment should have been trimmed, so the queue is empty - note we lost some writes from
	// the trimmed segment!
