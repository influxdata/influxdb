...
func assertReplicationsEqual(t *testing.T, want, got influxdb.Replication) {
	t.Helper()
	// Do not compare IDs, since these are randomly generated by the store methods.
	want.ID = got.ID
...
func TestDeleteReplication(t *testing.T) {
	...
	created, err := testStore.CreateReplication(ctx, initID, createReq)
	require.NoError(t, err)
	assertReplicationsEqual(t, replication, *created)
