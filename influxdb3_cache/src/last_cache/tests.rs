...
use crate::{
    last_cache::{
        ...
        cache::{KeyValue, LastCache, LastCacheKeyColumnsArg, LastCacheValueColumnsArg, Predicate},
    },
...
async fn cache_key_column_predicates() {
    ...
    let test_cases = [
        ...
        // the value columns:
        TestCase {
            predicates: predicates([(region_col_id, Predicate::new_in([KeyValue::string("us")]))]),
            expected: &[
                "+--------+------+-----------------------------+-------+",
                "| region | host | time                        | usage |",
                "+--------+------+-----------------------------+-------+",
        ...
        TestCase {
            predicates: predicates([(region_col_id, Predicate::new_in([KeyValue::string("ca")]))]),
            expected: &[
                "+--------+------+-----------------------------+-------+",
                "| region | host | time                        | usage |",
                "+--------+------+-----------------------------+-------+",
        ...
        TestCase {
            predicates: predicates([(region_col_id, Predicate::new_in([KeyValue::string("eu")]))]),
            expected: &["++", "++"],
        },
...
async fn fields_as_key_columns() {
    ...
    let test_cases = [
        ...
        TestCase {
            predicates: predicates([(active_col_id, Predicate::new_in([KeyValue::Bool(false)]))]),
            expected: &[
                "+--------------+--------+-------------+---------+---------+-----------------------------+",
                "| component_id | active | type        | loc     | reading | time                        |",
                "+--------------+--------+-------------+---------+---------+-----------------------------+",
...
async fn series_key_as_default() {
    ...
    let test_cases = [
        ...
        TestCase {
            predicates: predicates([(state_col_id, Predicate::new_in([KeyValue::string("ca")]))]),
            expected: &[
                "+-------+--------+-------+-------+-----------------------------+",
                "| state | county | farm  | speed | time                        |",
                "+-------+--------+-------+-------+-----------------------------+",
        ...
        TestCase {
            predicates: predicates([(farm_col_id, Predicate::new_in([KeyValue::string("30-01")]))]),
            expected: &[
                "+-------+--------+-------+-------+-----------------------------+",
                "| state | county | farm  | speed | time                        |",
                "+-------+--------+-------+-------+-----------------------------+",
...
async fn new_fields_added_to_default_cache() {
    ...
    let test_cases = [
        ...
        TestCase {
            predicates: predicates([(game_id_col_id, Predicate::new_in([KeyValue::string("4")]))]),
            expected: &[
                "+---------+-----------+-----------------------------+------+------+",
        ...
        TestCase {
            predicates: predicates([(game_id_col_id, Predicate::new_in([KeyValue::string("1")]))]),
            expected: &[
                "+---------+-----------+-----------------------------+------+------+",
                "| game_id | player    | time                        | type | zone |",
                "+---------+-----------+-----------------------------+------+------+",
