...
"#,
    ...
    fn create_predicate_map(predicates: &[(ColumnId, Predicate)]) -> IndexMap<ColumnId, Predicate> {
...
async fn test_row_with_nulls_not_ignored() {
    ...
    let distinct_cache_provider =
        DistinctCacheProvider::new_from_catalog(Arc::clone(&time_provider) as _, Arc::clone(&cat))
            .await
            .unwrap();
...
async fn test_distinct_with_where_clause_bug() {
    ...
    let dvc =
        DistinctCacheProvider::new_from_catalog(writer.catalog().time_provider(), writer.catalog())
            .await
            .unwrap();
