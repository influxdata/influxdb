...
function useResource<T>(resource: Resource<T>, onChange: ResourceUpdater<T>, generator?: Generator<T>): ResourceManipulator<T> {
    return {
        get: useCallback((id: DataID<T>): T => {
        ...
        add: useCallback((id: DataID<T>, data?: T) => {
            if (data) {
        ...
        update: useCallback((id: DataID<T>, data: Partial<T>) => {
            if (!resource.byID.hasOwnProperty(id)) {
        ...
        remove: useCallback((id: DataID<T>) => {
            if (!resource.byID.hasOwnProperty(id)) {
                return
            }
        ...
        indexOf: useCallback((id: DataID<T>):number => {
            return resource.allIDs.indexOf(id)
        }, [resource]),
        move: useCallback((_id: DataID<T>, _index: number) => {
