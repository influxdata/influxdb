...
func (l *Log) initWriter(w io.Writer, id, term, index uint64) (*logWriter, error) {
	...
	//   3. Index is after the last entry's index.
	if l.state != Leader {
		...
		return nil, fmt.Errorf("entry not found: follower=%d, leader=%d", index, l.lastLogIndex)
	}
