...
EXPLORER_PORT=8888  # Can be changed if port is in use (Docker)
...
# Explorer UI Binary Configuration
# ─────────────────────────────────────────────────────────────────────────────
# For local testing, override these via environment variables:
#
#   EXPLORER_BINARY_DIR   - Directory containing the Explorer binary
#   EXPLORER_BINARY_NAME  - Name of the executable (default: influxdb3-explorer)
#   EXPLORER_BINARY_PORT  - Port to run Explorer on (default: 3000)
#   EXPLORER_DATA_DIR     - Directory for Explorer data (default: ~/.influxdb/explorer-data)
#
# Example (testing with local build):
#   EXPLORER_BINARY_DIR=~/code/influxdb3-ui/dist/target/release \
#   EXPLORER_BINARY_NAME=rust_standalone \
#   ./install_influxdb.sh
# ─────────────────────────────────────────────────────────────────────────────
EXPLORER_BINARY_PORT="${EXPLORER_BINARY_PORT:-3000}"
EXPLORER_BINARY_DIR="${EXPLORER_BINARY_DIR:-$INSTALL_LOC}"
EXPLORER_BINARY_NAME="${EXPLORER_BINARY_NAME:-influxdb3-explorer}"
EXPLORER_BINARY_PATH="${EXPLORER_BINARY_DIR}/${EXPLORER_BINARY_NAME}"
EXPLORER_DATA_DIR="${EXPLORER_DATA_DIR:-$INSTALL_LOC/explorer-data}"
EXPLORER_BINARY_STARTED=false

# Process IDs for cleanup
PID=""
EXPLORER_PID=""

# Cleanup function for Ctrl+C or script exit
cleanup_processes() {
...
# Parameters: $1 - timeout (default 60), $2 - port (default $EXPLORER_PORT)
wait_for_explorer_ready() {
    TIMEOUT="${1:-60}"
    CHECK_PORT="${2:-$EXPLORER_PORT}"
    ...
    while [ $ELAPSED -lt $TIMEOUT ]; do
        if curl -s http://localhost:$CHECK_PORT >/dev/null 2>&1; then
            API_RESPONSE=$(curl -s http://localhost:$CHECK_PORT/api/health 2>&1)
...
# --- Explorer Binary Configuration ---

# Function to get the Explorer UI config file path based on OS
# Returns: Path to config.toml (echoes to stdout)
get_explorer_config_path() {
    if [ "${OS}" = "Darwin" ]; then
        echo "$HOME/Library/Application Support/influxdb3-ui/config.toml"
    else
        # Linux
        echo "$HOME/.config/influxdb3-ui/config.toml"
    fi
}

# Function to save Explorer UI configuration to TOML file
# Parameters:
#   $1 - api_token: The API token to save
#   $2 - db_host: The InfluxDB host URL (e.g., http://localhost:8181)
#   $3 - server_name: Optional server name (default: "InfluxDB 3")
# Creates config directory if needed and writes config.toml
save_explorer_config() {
    api_token="$1"
    db_host="$2"
    server_name="${3:-InfluxDB 3}"

    config_path=$(get_explorer_config_path)
...
# InfluxDB 3 UI Configuration
# Generated by install_influxdb.sh
...
port = $EXPLORER_BINARY_PORT

# Log level: error, warn, info, trace (default: info)
log_level = "info"
CONFIG_EOF

    # Set restrictive permissions on config file (contains token)
    chmod 600 "$config_path"

    printf "├─${DIM} Saved Explorer config: %s${NC}\n" "$config_path"
}

# --- Explorer Binary Management ---

# Function to start Explorer binary
# Parameters: $1 - InfluxDB port to connect to
start_explorer_binary() {
    INFLUXDB_CONNECT_PORT="${1:-8181}"

    # Check if Explorer binary exists
    if [ ! -x "$EXPLORER_BINARY_PATH" ]; then
        printf "├─${YELLOW} Explorer binary not found at %s${NC}\n" "$EXPLORER_BINARY_PATH"
        printf "├─${YELLOW} Skipping Explorer startup${NC}\n"
        return 1
    fi

    # Create Explorer data directory (uses global EXPLORER_DATA_DIR)
    mkdir -p "$EXPLORER_DATA_DIR"

    # Create Explorer log file
    EXPLORER_LOG_FILE="$INSTALL_LOC/logs/explorer_$(date +%Y%m%d_%H%M%S).log"

    printf "├─${DIM} Explorer port: %s${NC}\n" "$EXPLORER_BINARY_PORT"
    printf "├─${DIM} Explorer data: %s${NC}\n" "$EXPLORER_DATA_DIR"
    printf "├─${DIM} Explorer logs: %s${NC}\n" "$EXPLORER_LOG_FILE"
    printf "├─${DIM} Connecting to InfluxDB at http://localhost:%s${NC}\n" "$INFLUXDB_CONNECT_PORT"

    # Start Explorer binary in background
    "$EXPLORER_BINARY_PATH" \
        --port "$EXPLORER_BINARY_PORT" \
        --db-host "http://localhost:$INFLUXDB_CONNECT_PORT" \
        --data-dir "$EXPLORER_DATA_DIR" \
        >> "$EXPLORER_LOG_FILE" 2>&1 &
    EXPLORER_PID="$!"

    printf "├─${DIM} Explorer started (PID: %s)${NC}\n" "$EXPLORER_PID"

    # Wait for Explorer to be ready
    if wait_for_explorer_ready 30 "$EXPLORER_BINARY_PORT"; then
        printf "\n${BOLDGREEN}✓ Explorer is running at http://localhost:%s${NC}\n" "$EXPLORER_BINARY_PORT"
        open_browser_url "http://localhost:${EXPLORER_BINARY_PORT}"
        EXPLORER_BINARY_STARTED=true
        return 0
    else
        printf "├─${YELLOW} Explorer failed to start within timeout${NC}\n"
        printf "├─${YELLOW} Check logs at: %s${NC}\n" "$EXPLORER_LOG_FILE"
        return 1
    fi
}

# Function to create operator token via API for binary installation
# Parameters: $1 - InfluxDB port
# Returns: Token string on success, error message on failure
create_binary_operator_token() {
    INFLUXDB_CONNECT_PORT="${1:-8181}"
    ...
    while [ $RETRY -lt $MAX_RETRIES ] && [ -z "$TOKEN" ]; do
        TOKEN_RESPONSE=$(curl -s -w "\n%{http_code}" -m 5 -X POST "http://localhost:$INFLUXDB_CONNECT_PORT/api/v3/configure/token/admin" 2>&1)
        ...
        # Check for 409 conflict (token already exists)
        if [ "$HTTP_CODE" = "409" ]; then
            echo "TOKEN_ALREADY_EXISTS"
            return 2
        fi
...
# Function to prompt user about Explorer and start if desired
# Parameters: $1 - InfluxDB port
prompt_and_start_explorer() {
    INFLUXDB_CONNECT_PORT="${1:-8181}"

    # Check if Explorer binary exists before prompting
    if [ ! -f "$EXPLORER_BINARY_PATH" ]; then
        printf "\n"
        printf "${DIM}Explorer binary not found at %s${NC}\n" "$EXPLORER_BINARY_PATH"
        printf "${DIM}To use Explorer UI, download it from: https://docs.influxdata.com/influxdb3/explorer/${NC}\n"
        return 0
    fi

    printf "\n"
    printf "${BOLD}Start Explorer UI?${NC}\n"
    printf "├─ Explorer provides a web UI for querying, plugins, and management\n"
    printf "├─ 1) ${GREEN}Yes${NC} ${DIM}(recommended)${NC}\n"
    printf "├─ 2) ${GREEN}No${NC} ${DIM}(install only)${NC}\n"
    printf "└─ Enter your choice (1-2): "
    read -r START_EXPLORER
    START_EXPLORER=${START_EXPLORER:-1}

    case "$START_EXPLORER" in
        1)
            printf "\n${BOLD}Setting up Explorer${NC}\n"

            # Create operator token for Explorer (disable exit-on-error temporarily)
            printf "├─ Creating operator token...\n"
            set +e
            EXPLORER_TOKEN=$(create_binary_operator_token "$INFLUXDB_CONNECT_PORT" 2>&1)
            TOKEN_RESULT=$?
            set -e

            if [ $TOKEN_RESULT -eq 0 ]; then
                # Token created successfully - display and save
                printf "│\n"
                printf "${BOLD}════════════════════════════════════════════════════════════${NC}\n"
                printf "${BOLD}OPERATOR TOKEN: Save this token. It will not be shown again.${NC}\n"
                printf "%s\n" "$EXPLORER_TOKEN"
                ...
                # Save to Explorer config file
                printf "├─ Saving Explorer configuration...\n"
                save_explorer_config "$EXPLORER_TOKEN" "http://localhost:$INFLUXDB_CONNECT_PORT" "InfluxDB 3 ${EDITION}"

                # Verify config file was created
                CONFIG_PATH=$(get_explorer_config_path)
                if [ -f "$CONFIG_PATH" ]; then
                    printf "├─${GREEN} Config file created successfully${NC}\n"
                    # printf "├─${DIM} Contents:${NC}\n"
                    # sed 's/^/│    /' "$CONFIG_PATH"
                    # printf "│\n"
                else
                    printf "├─${RED} Warning: Config file was not created${NC}\n"
                fi

            elif [ $TOKEN_RESULT -eq 2 ]; then
                # Token already exists - check for existing config
                printf "├─${DIM} Operator token already exists${NC}\n"
                CONFIG_PATH=$(get_explorer_config_path)
                if [ -f "$CONFIG_PATH" ]; then
                    printf "├─${DIM} Using existing config: %s${NC}\n" "$CONFIG_PATH"
                else
                    printf "├─${YELLOW} No config file found. Create token manually and add to config.${NC}\n"
                    printf "├─${DIM} Config location: %s${NC}\n" "$CONFIG_PATH"
                fi
            else
                # Token creation failed - skip Explorer startup
                printf "├─${YELLOW} Could not create operator token automatically${NC}\n"
                printf "├─ Create token manually: ${BOLD}influxdb3 create token --admin${NC}\n"
                CONFIG_PATH=$(get_explorer_config_path)
                printf "├─ Then add to config: ${DIM}%s${NC}\n" "$CONFIG_PATH"
                printf "└─ Skipping Explorer startup (no token available)\n"
                return 1
            fi

            printf "│\n"
            printf "${BOLD}Starting Explorer${NC}\n"
            start_explorer_binary "$INFLUXDB_CONNECT_PORT"
            return $?
            ;;
        2|*)
            printf "\n├─ Skipping Explorer startup\n"
            printf "├─ You can start Explorer later by running:\n"
            printf "└─${DIM} %s --port %s --db-host http://localhost:%s --data-dir %s${NC}\n" "$EXPLORER_BINARY_PATH" "$EXPLORER_BINARY_PORT" "$INFLUXDB_CONNECT_PORT" "$EXPLORER_DATA_DIR"
            return 0
            ;;
    esac
}
...
if [ "${EDITION}" = "Core" ]; then
    ...
    if [ "$START_SERVICE" = "y" ] && [ "$STARTUP_CHOICE" = "2" ]; then
        ...
        # Prompt to start Explorer after InfluxDB is running (only if startup succeeded)
        if [ "${SUCCESS:-0}" -eq 1 ]; then
            prompt_and_start_explorer "$PORT"
        fi
...
if [ "${EDITION}" = "Enterprise" ] && [ "$SUCCESS" -eq 0 ] 2>/dev/null; then
    ...
    if [ "$EXPLORER_BINARY_STARTED" = "false" ]; then
...
# Only show Explorer advertisement if Explorer wasn't already started
if [ "$EXPLORER_BINARY_STARTED" = "false" ]; then
    printf "┌────────────────────────────────────────────────────────────────────────────────────────┐\n"
