...
func KVStore(
	init func(KVStoreFields, *testing.T) (Store, func()),
	t *testing.T,
) {
	tests := []struct {
		name string
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := s.Update(context.Background(), func(tx Tx) error {
...

				{
					...
					if _, err := b.Get(tt.args.key); err != ErrKeyNotFound {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				seek:  "aaa",
				until: "aaa/03",
				hints: []CursorHint{WithCursorHintPrefix("aaa/")},
			},
		...
		},
		{
			...
			args: args{
				seek:  "aaa",
				until: "bbb/00",
				hints: []CursorHint{WithCursorHintKeyStart("aaa/")},
			},
			exp: []string{"aaa/00", "aaa/01", "aaa/02", "aaa/03", "bbb/00"},
		},
		{
			...
			args: args{
				...
				until: "aaa/03",
				hints: []CursorHint{
					WithCursorHintPredicate(func(key, _ []byte) bool {
		...
		},
		{
			...
			args: args{
				...
				until: "aa/01",
				hints: []CursorHint{
					WithCursorHintPredicate(func(_, val []byte) bool {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				until: "bbb/02",
				opts: []CursorOption{
					WithCursorPrefix([]byte("aab")),
				},
			},
			expErr: ErrSeekMissingPrefix,
		},
		{
			name: "prefix hint",
			fields: KVStoreFields{
			...
			args: args{
				...
				until: "aaa/03",
				opts: []CursorOption{
					WithCursorHints(WithCursorHintPrefix("aaa/")),
				},
			},
			exp: []string{"aaa/00", "aaa/01", "aaa/02", "aaa/03"},
		},
		{
			...
			args: args{
				...
				until: "bbb/00",
				opts: []CursorOption{
					WithCursorHints(WithCursorHintKeyStart("aaa/")),
				},
			},
			exp: []string{"aaa/00", "aaa/01", "aaa/02", "aaa/03", "bbb/00"},
		},
		{
			...
			args: args{
				...
				until: "aaa/03",
				opts: []CursorOption{
					WithCursorHints(WithCursorHintPredicate(func(key, _ []byte) bool {
						return len(key) < 3 || string(key[:3]) == "aaa"
					})),
				},
			},
		...
		},
		{
			...
			args: args{
				...
				until: "aa/01",
				opts: []CursorOption{
					WithCursorHints(WithCursorHintPredicate(func(_, val []byte) bool {
						return len(val) < 7 || string(val[:7]) == "val:aa/"
					})),
				},
			},
		...
		},
		{
			...
			args: args{
				seek:  "bbb/00",
				until: "aaa/00",
				opts:  []CursorOption{WithCursorDirection(CursorDescending)},
			},
		...
		},
		{
			...
			args: args{
				...
				opts: []CursorOption{
					WithCursorPrefix([]byte("aaa/")),
					WithCursorDirection(CursorDescending),
				},
			},
