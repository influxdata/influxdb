...
use crate::google::{FieldViolation, FromField, OptionalField};
...
impl TryFrom<management::Sink> for Sink {
    ...
    fn try_from(proto: management::Sink) -> Result<Self, Self::Error> {
        Ok(match proto.sink.unwrap_field("")? {
            management::sink::Sink::Iox(node_group) => Sink::Iox(node_group.field("node_group")?),
            management::sink::Sink::Kafka(kafka) => Sink::Kafka(kafka.field("kafka")?),
