...
impl FromStr for DisableAuthzList {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        for r in &resources {
            if !["health", "ping", "metrics", "ready", "pprof", "node_stop"]
                .iter()
        ...
        let resources_static = resources
            ...
            .flat_map(|path| match path.as_str() {
                "health" => vec![all_paths::API_V3_HEALTH, all_paths::API_V1_HEALTH],
                "ping" => vec![all_paths::API_PING],
                "metrics" => vec![all_paths::API_METRICS],
                "ready" => vec![all_paths::API_V3_READY],
                "pprof" => vec![all_paths::API_DEBUG_PPROF_HEAP],
                "node_stop" => vec![API_V3_ENTERPRISE_CONFIGURE_NODE_STOP],
                _ => vec![],
            })
            .collect();
