...
func (self *ServerSuite) queryServer(server *Server, database, query string, c *C) []interface{} {
	encodedQuery := url.QueryEscape(query)
	fullUrl := fmt.Sprintf("http://localhost:%d/db/%s/series?u=paul&p=pass&q=%s", server.Config.ApiHttpPort, database, encodedQuery)
...
func (self *ServerSuite) TestColumnNamesReturnInDistributedQuery(c *C) {
	...
	time.Sleep(time.Millisecond * 100)
	response := self.queryServer(self.servers[0], "test_rep", "select * from cluster_query_with_columns limit 1", c)
