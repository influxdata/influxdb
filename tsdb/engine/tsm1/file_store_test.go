...
func newFiles(tb testing.TB, dir string, values ...keyValues) ([]string, error) {
	return newFilesId(tb, dir, 1, values...)
}
...
func newFilesId(tb testing.TB, dir string, id int, values ...keyValues) ([]string, error) {
	var files []string
			...
			go func() {
				...
				for {
					...
					case <-done:
						return
					default:
						assert.NoError(b, fs.Apply(b.Context(), func(r tsm1.TSMFile) error {
			...
			go func() {
				...
				for {
					...
					default:
						...
						id++
						files, err := newFilesId(b, churn, id, churnData...)
...
func TestFileStore_ConcurrentLocking(t *testing.T) {
	...
	files, err := newFiles(t, dir, keyValues{"cpu", []tsm1.Value{tsm1.NewValue(0, 1.0)}})
	require.NoError(t, err)
	...
	go func() {
		...
		for j := 0; j < iters; j++ {
			bump()
			nf, err := newFilesId(t, churnDir, j+2, keyValues{"mem", []tsm1.Value{tsm1.NewValue(int64(j), 2.0)}})
			if assert.NoError(t, err) {
