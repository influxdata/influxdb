...
const (
	...
	timeCompressedRLE = 2
	// timeCompressedFromZero is based on timeCompressedPackedSimple, and the deltas are starting from 0
	timeCompressedFromZero = 3
)
...
func (e *encoder) encodePacked(div, min uint64, dts []uint64) ([]byte, error) {
	...
	b[0] = byte(timeCompressedFromZero) << 4
...
func (d *TimeDecoder) decode(b []byte) {
	...
	case timeCompressedPackedSimple:
		d.decodePacked(b)
	case timeCompressedFromZero:
		d.decodeFromZeroPacked(b)
	default:
...
func (d *TimeDecoder) decodeFromZeroPacked(b []byte) {
	if len(b) < 17 {
		d.err = fmt.Errorf("TimeDecoder: not enough data to decode packed timestamps")
		return
	}
