...
func (e *Engine) HasValueForSeriesInTimeRange(ctx context.Context, seriesID uint64, start, stop int64) (bool, error) {
	foundAny := false

	seriesKey := e.sfile.SeriesKey(seriesID)
	measurementName, tags := tsdb.ParseSeriesKey(seriesKey)
	mf := e.MeasurementFields(measurementName)

	q, err := e.CreateCursorIterator(ctx)
	if err != nil {
		return false, err
	}

	mf.ForEachField(func(fieldName string, typ influxql.DataType) bool {
		cur, err := q.Next(ctx, &tsdb.CursorRequest{
			Name:      measurementName,
			Tags:      tags,
			Field:     fieldName,
			Ascending: true,
			StartTime: start,
			EndTime:   stop - 1, // end time is exclusive
		})
		if err != nil {
			return false
		}
