...
type SeriesPartition struct {
	...
	stats *SeriesPartitionStats // generated by compaction
...
func (p *SeriesPartition) CreateSeriesListIfNotExists(keys [][]byte, keyPartitionIDs []int, ids []uint64) error {
	...
	if p.compactionsEnabled() && !p.compacting && p.CompactThreshold != 0 && p.index.InMemCount() >= uint64(p.CompactThreshold) {
		...
		go func() {
			...
			// Clear compaction flag & update stats.
			p.mu.Lock()
			p.stats = compactor.Stats
			p.compacting = false
			p.mu.Unlock()
		}()
	}
...
// SeriesPartitionStats returns stats generated by compaction.
func (p *SeriesPartition) Stats() *SeriesPartitionStats {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.stats
}
...
type SeriesPartitionCompactor struct {
	...
	// Available after compaction is complete.
	Stats *SeriesPartitionStats
}
...
// If previous stats are available then segments can be compacted.
func (c *SeriesPartitionCompactor) Compact(p *SeriesPartition) error {
	...
	// TODO(series-file-compaction): Compact segments over threshold based on previous stats.
...
func (c *SeriesPartitionCompactor) compactIndexTo(index *SeriesIndex, seriesN uint64, segments []*SeriesSegment, path string) error {
	...
	// Track series id inserts to determine orphaned tombstones.
	...
	for _, segment := range segments {
		...
		sstats := &SeriesSegmentStats{}
		stats.SegmentStats[segment.ID()] = sstats
		...
		if err := segment.ForEachEntry(func(flag uint8, id uint64, offset int64, key []byte) error {
			...
			case SeriesEntryTombstoneFlag:
				sstats.TombstoneN++
				if !seriesIDs.ContainsNoLock(id) {
					sstats.OrphanTombstoneN++
				}
				return nil
			default:
			...
			// Track series count and ids.
			sstats.SeriesN++
			seriesIDs.AddNoLock(id)

			// Ignore entry if tombstoned.
			if index.IsDeleted(id) {
				sstats.DeletedSeriesN++
				return nil
			}
...
// SeriesPartitionStats tracks series and tombstone stats for a partition and its segments.
type SeriesPartitionStats struct {
	SegmentStats map[uint16]*SeriesSegmentStats
}

// NewSeriesPartitionStats returns a new instance of SeriesPartitionStats.
func NewSeriesPartitionStats() *SeriesPartitionStats {
	return &SeriesPartitionStats{
		SegmentStats: make(map[uint16]*SeriesSegmentStats),
	}
}
