...
// NewDatabaseIndex makes a new DataBaseIndex.
...
func (d *DatabaseIndex) RewriteSelectStatement(stmt *influxql.SelectStatement) (*influxql.SelectStatement, error) {
	// Expand regex expressions in the FROM clause.
	sources, err := d.ExpandSources(stmt.Sources)
	if err != nil {
		return nil, err
	}
	...
	stmt, err = d.ExpandWildcards(stmt)
	if err != nil {
		return nil, err
	}
...
func (d *DatabaseIndex) ExpandWildcards(stmt *influxql.SelectStatement) (*influxql.SelectStatement, error) {
...
func (d *DatabaseIndex) ExpandSources(sources influxql.Sources) (influxql.Sources, error) {
	...
	for _, source := range sources {
		...
		case *influxql.Measurement:
			...
			measurements := d.measurementsByRegex(src.Regex.Val)
...
// SelectInfo represents fields and tags in a select statement.
...
// InitializeShards initializes the shardIDs in a Series.
...
// MarshalTags is used to convert the tag set to bytes for use as a lookup key.
...
// MeasurementFromSeriesKey returns the measurement part of a
// series key string (everything in the string up to ',', if any).
