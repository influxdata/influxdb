...
// Check is required to implement the ProxyQueryService interface, but there is no associated handler for it.
func (s *SourceProxyQueryService) Check(context.Context) check.Response {
	return check.Response{Name: "SourceProxyQueryService", Status: check.StatusPass}
}
