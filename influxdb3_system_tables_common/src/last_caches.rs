//! `system.last_caches` system table implementation [`LastCachesTable`]
use std::sync::Arc;
...
use influxdb3_catalog::catalog::{DatabaseSchema, LastCacheDefinition, LastCacheValueColumnsDef};
use iox_system_tables::IoxSystemTable;

/// `system.last_caches` system table implementation
#[derive(Debug)]
...
fn from_last_cache_definitions(
    ...
    for cache_defn in cache_defns {
        ...
        let mut key_col_ids = Vec::with_capacity(cache_defn.key_columns.len());
        let mut have_all_key_ids = true;
        for key_col in &cache_defn.key_columns {
            ...
            if let Some(key_col_id) = table_defn.columns.id_to_ord_id(key_col) {
                key_col_ids.push(key_col_id.get());
            } else {
                have_all_key_ids = false;
            }
            key_col_names_arr.values().append_value(col_name);
        }
        if have_all_key_ids {
            for key_col_id in key_col_ids {
                key_col_ids_arr.values().append_value(key_col_id);
            }
            key_col_ids_arr.append(true);
        } else {
            key_col_ids_arr.append_null();
        }
        key_col_names_arr.append(true);

        match &cache_defn.value_columns {
            LastCacheValueColumnsDef::Explicit { columns } => {
                let mut value_col_ids = Vec::with_capacity(columns.len());
                let mut have_all_value_ids = true;
                for col in columns {
                    let col_name = table_defn
                        .column_id_to_name(col)
                        .expect("column id should have name associated to it");
                    if let Some(col_id) = table_defn.columns.id_to_ord_id(col) {
                        value_col_ids.push(col_id.get());
                    } else {
                        have_all_value_ids = false;
                    }
                    value_col_names_arr.values().append_value(col_name);
                }
                if have_all_value_ids {
                    for col_id in value_col_ids {
                        value_col_ids_arr.values().append_value(col_id);
                    }
