//! `system.nodes` system table implementation [`NodeSystemTable`]
use std::sync::Arc;
...
/// `system.nodes` system table implementation
#[derive(Debug)]
pub struct NodeSystemTable {
    catalog: Arc<Catalog>,
    schema: SchemaRef,
}
...
fn table_schema() -> SchemaRef {
    let fields = vec![
        ...
        Field::new("conn_info", DataType::Utf8View, true),
...
fn to_record_batch(
    ...
    let mut conn_info_builder = StringViewBuilder::with_capacity(cap);
    ...
    for node in &nodes {
        ...
        if let Some(conn_info) = node.conn_info() {
            conn_info_builder.append_value(conn_info);
        } else {
            conn_info_builder.append_null();
        }
    ...
    RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(conn_info_builder.finish()),
