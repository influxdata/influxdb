//! `system.distinct_caches` system table implementation [`DistinctCachesTable`]
use std::sync::Arc;
...
/// `system.distinct_caches` system table implementation
#[derive(Debug)]
...
impl DistinctCachesTable {
    pub fn new(db_schema: Arc<DatabaseSchema>) -> Self {
        Self {
            db_schema,
...
fn from_distinct_cache_definitions(
    ...
    for cache in cache_definitions {
        ...
        let mut col_ids = Vec::with_capacity(cache.column_ids.len());
        let mut have_all_col_ids = true;
        for col in &cache.column_ids {
            let col_name = table_def
                .column_id_to_name(col)
                .expect("column id should have associated name");
            if let Some(col_id) = table_def.columns.id_to_ord_id(col) {
                col_ids.push(col_id.get());
            } else {
                have_all_col_ids = false;
            }
            col_name_arr.values().append_value(col_name);
        }
        if have_all_col_ids {
            for col_id in col_ids {
                col_id_arr.values().append_value(col_id);
            }
            col_id_arr.append(true);
        } else {
            col_id_arr.append_null();
        }
