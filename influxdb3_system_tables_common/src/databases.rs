//! `system.databases` system table implementation [`DatabasesTable`]
use std::sync::Arc;
...
/// `system.databases` system table implementation
#[derive(Debug)]
...
fn databases_schema() -> SchemaRef {
    let columns = vec![
        ...
        Field::new("schema_mode", DataType::Utf8View, false),
    ];
...
impl IoxSystemTable for DatabasesTable {
    ...
    async fn scan(
        ...
        let mut schema_mode_arr = StringViewBuilder::with_capacity(databases.len());
        ...
        for db in databases {
            ...
            schema_mode_arr.append_value(db.schema_mode.as_str());
        ...
        let columns: Vec<ArrayRef> = vec![
            ...
            Arc::new(schema_mode_arr.finish()),
        ];
