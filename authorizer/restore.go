...
func (b RestoreService) RestoreBucket(ctx context.Context, id platform.ID, dbi []byte, replace bool, update *influxdb.RestoredBucketUpdate) (shardIDMap map[uint64]uint64, err error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()
	...
	return b.s.RestoreBucket(ctx, id, dbi, replace, update)
}
