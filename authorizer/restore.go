...
func (b RestoreService) RestoreBucket(ctx context.Context, id platform.ID, dbi []byte, replace bool, onReplaceCommitted func(context.Context) error) (shardIDMap map[uint64]uint64, err error) {
	span, ctx := tracing.StartSpanFromContext(ctx)
	defer span.Finish()
