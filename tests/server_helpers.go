...
func NewRetentionPolicySpec(name string, rf int, duration time.Duration, futureWriteLimit time.Duration, pastWriteLimit time.Duration) *meta.RetentionPolicySpec {
	return &meta.RetentionPolicySpec{Name: name, ReplicaN: &rf, Duration: &duration, FutureWriteLimit: &futureWriteLimit, PastWriteLimit: &pastWriteLimit}
}
...
func writeTestData(s Server, t *Test) error {
	for i, w := range t.writes {
		...
		if err := s.CreateDatabaseAndRetentionPolicy(w.db, NewRetentionPolicySpec(w.rp, 1, 0, 0, 0), true); err != nil {
			return err
		}
