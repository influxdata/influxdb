...
func TestServer_Query_Chunk(t *testing.T) {
	...
	writes := make([]string, 101) // 10,000 is the default chunking size, even when no chunking requested.
	expectedValues := make([]string, config.HTTPD.MaxRowLimit)
