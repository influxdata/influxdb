    ...
    T: TimeProvider,
{
    ...
    /// Endpoint for testing a plugin that will be trigger on WAL writes.
    #[cfg(feature = "system-py")]
    async fn test_processing_engine_cron_plugin(
        &self,
        req: Request<Body>,
    ) -> Result<Response<Body>> {
        let request: influxdb3_client::plugin_development::CronPluginTestRequest =
            self.read_body_json(req).await?;

        let output = self
            .processing_engine
            .test_cron_plugin(request, Arc::clone(&self.query_executor))
            .await?;
        let body = serde_json::to_string(&output)?;
...
pub(crate) async fn route_request<T: TimeProvider>(
    ...
    let response = match (method.clone(), uri.path()) {
        ...
        (Method::POST, "/api/v3/plugin_test/cron") => {
            http_server.test_processing_engine_cron_plugin(req).await
        }
        _ => {
