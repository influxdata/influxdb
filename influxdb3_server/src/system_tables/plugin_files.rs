...
use influxdb3_processing_engine::ProcessingEngine;
...
pub(super) struct PluginFilesTable {
    schema: SchemaRef,
    processing_engine: Option<Arc<ProcessingEngine>>,
}

impl PluginFilesTable {
    pub(super) fn new(processing_engine: Option<Arc<ProcessingEngine>>) -> Self {
        Self {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_plugin_files_table_empty() {
        let table = PluginFilesTable::new(None);
