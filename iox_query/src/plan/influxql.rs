...
impl<'a> InfluxQLToLogicalPlan<'a> {
    ...
    async fn select_statement_to_plan(&self, select: SelectStatement) -> Result<LogicalPlan> {
        ...
        let plan = self.plan_where_clause(select.condition, plan)?;
    ...
    fn conditional_to_df_expr(
        ...
        match iql {
            ...
            ConditionalExpression::Binary { lhs, op, rhs } => {
                self.binary_conditional_to_df_expr(lhs, *op, rhs, schema)
            }
    ...
    fn binary_conditional_to_df_expr(
        ...
        Ok(Expr::BinaryExpr(BinaryExpr::new(
            Box::new(self.conditional_to_df_expr(lhs, schema)?),
            match op {
            ...
            Box::new(self.conditional_to_df_expr(rhs, schema)?),
        )))
    }
    ...
    fn plan_where_clause(
        ...
        match condition {
            Some(where_clause) => {
                let input_schema = plan.schema();
                let filter_expr = self.conditional_to_df_expr(&where_clause, input_schema)?;
