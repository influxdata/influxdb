...
func (m *Launcher) run(ctx context.Context) (err error) {
	...
	var (
		...
		/*
			...
			bucketLogSvc     platform.BucketOperationLogService       = m.svc
			orgLogSvc        platform.OrganizationOperationLogService = m.svc
		*/
		onboardingSvc    platform.OnboardingService          = m.svc
		scraperTargetSvc platform.ScraperTargetStoreService  = m.svc
		telegrafSvc      platform.TelegrafConfigStore        = m.svc
		userResourceSvc  platform.UserResourceMappingService = m.svc
		labelSvc         platform.LabelService               = m.svc
		//secretSvc        platform.SecretService                   = m.svc
		lookupSvc platform.LookupService = m.svc
	)
	...
	var taskSvc platform.TaskService
	{
		...
		executor := taskexecutor.NewAsyncQueryServiceExecutor(m.logger.With(zap.String("service", "task-executor")), m.queryController, store)
		...
		taskSvc = task.PlatformAdapter(coordinator.New(m.logger.With(zap.String("service", "task-coordinator")), m.scheduler, store), lr, m.scheduler)
