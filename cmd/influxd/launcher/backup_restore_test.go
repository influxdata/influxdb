...
func TestBackupRestore_Full(t *testing.T) {
	...
	// Check that buckets were reset to match the original server's metadata.
	...
	_, err = l2.BucketService(t).FindBucket(ctx, influxdb.BucketFilter{OrganizationID: &l1.Org.ID, ID: &b2.ID})
