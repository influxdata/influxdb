...
// Ensure the server correct supports data with identical tag values.
func TestServer_Query_IdenticalTagValues(t *testing.T) {
	...
	test.addQueries([]*Query{
		&Query{
			name:    "measurements with identical tag values",
			command: `SELECT * FROM db0.rp0.cpu`,
			exp:     `{"results":[{"series":[{"name":"cpu","columns":["time","value"],"values":[["2000-01-01T00:00:00Z",1],["2000-01-01T00:00:00Z",2]]}]}]}`,
		},
		&Query{
			name:    "measurements with identical tag values",
