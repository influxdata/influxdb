...
impl ProcessingEngineManagerImpl {
    ...
    pub async fn update_plugin_file(
        ...
        // Only allow normal path components (no .., /, absolute paths, etc.)
        let file_path_check = std::path::Path::new(file_name);
        for component in file_path_check.components() {
            match component {
                ...
                _ => {
                    return Err(ProcessingEngineError::PluginError(
                        plugins::PluginError::AnyhowError(anyhow::anyhow!(
                            ...
                            File names must be relative paths without '..' or absolute path components.",
                            file_name
                        )),
                    ));
                }
        ...
        for db_schema in self.catalog.list_db_schema() {
                ...
                && let Some(ref plugin_dir) = self.environment_manager.plugin_dir
            {
                ...
                if plugin_path.is_dir() {
                    ...
                    // Additional security check: ensure the final path is still within plugin_path
                    let canonical_plugin = plugin_path.canonicalize().map_err(|e| {
                        ProcessingEngineError::PluginError(plugins::PluginError::ReadPluginError(e))
                    })?;

                    // Create parent directories if they don't exist (for nested files like utils/helper.py)
                    ...
                    // Verify the written file is within the plugin directory
                    let canonical_file = file_path.canonicalize().map_err(|e| {
                        ProcessingEngineError::PluginError(plugins::PluginError::ReadPluginError(e))
                    })?;

                    if !canonical_file.starts_with(&canonical_plugin) {
                        // Delete the file if it ended up outside the plugin directory
                        let _ = fs::remove_file(&file_path);
                        return Err(ProcessingEngineError::PluginError(
                            plugins::PluginError::AnyhowError(anyhow::anyhow!(
                                "Security violation: file would be written outside plugin directory"
                            )),
                        ));
                    }
                } else {
