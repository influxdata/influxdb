...
async fn spawn_test_scheduler(
    ...
    scheduler
        .register_trigger(TriggerRegistration {
            ...
            cancel,
            config: SchedulerConfig::new(16, trigger_definition.trigger_settings.run_async),
...
async fn disable_policy_keeps_runtime_when_auto_disable_fails() {
    ...
    scheduler
        .register_trigger(TriggerRegistration {
            ...
            trigger_definition,
            cancel: cancel.clone(),
            config: SchedulerConfig::new(16, false),
...
async fn scheduler_round_robins_workers_and_routes_cancellation() {
    ...
    scheduler
        .register_trigger(TriggerRegistration {
            ...
            cancel: CancellationToken::new(),
            config: SchedulerConfig::new(16, true),
...
async fn run_async_true_respects_concurrency_bound() {
    ...
    let disable_count = Arc::new(AtomicUsize::new(0));
    let (scheduler, key) = spawn_test_scheduler(
        trigger(ErrorBehavior::Log, true),
        worker.factory(),
        CancellationToken::new(),
        disable_count,
    )
    ...
    for _ in 0..(RUN_ASYNC_CONCURRENCY_LIMIT + 4) {
        scheduler
            .enqueue(TriggerInvocation::new(key, payload()))
            .await
            .unwrap();
    }
    while worker.attempts() < RUN_ASYNC_CONCURRENCY_LIMIT {
        worker.started.notified().await;
    }
    tokio::task::yield_now().await;
    assert_eq!(worker.attempts(), RUN_ASYNC_CONCURRENCY_LIMIT);
    assert_eq!(worker.max_active(), RUN_ASYNC_CONCURRENCY_LIMIT);
    worker.release.notify_waiters();
    scheduler.shutdown_trigger(key).await;
}
