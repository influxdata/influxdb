...
use crate::gen1_cleanup_handler::CleanupResult;
...
#[tokio::test]
async fn test_purge_compacted_removes_matching_files() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let cache = TableIndexCache::new(
        "test_node".to_string(),
        TableIndexCacheConfig::default(),
    ...
    // Create a table index with files 0, 1, 2, 3, 4
    let db_id = DbId::from(1);
    let table_id = TableId::from(10);
    let table_index_id = TableIndexId::new("test_node", db_id, table_id);
    let index = create_test_index(table_index_id.clone(), 5);

    // Insert the index into the cache
    {
        let mut indices = cache.inner.indices.write().await;
        let cached = CachedTableIndex::new(index);
        indices
            .entry(("test_node".to_string(), db_id))
            .or_default()
            .insert(table_id, cached);
    }

    // Purge files 0 and 2 by exact ID
    let mut cleanup = CleanupResult::new();
    cleanup.removed_file_ids.insert(
        (db_id, table_id),
        vec![0u64, 2u64]
            .into_iter()
    ...
    let removed = cache
        .purge_compacted_for_tables("test_node", &cleanup, false)
        .await;

    assert_eq!(removed, 2, "Should have removed 2 files (0 and 2)");

    // Verify remaining files are 1, 3, 4
    let indices = cache.inner.indices.read().await;
    let cached = indices
        .get(&("test_node".to_string(), db_id))
        .unwrap()
        .get(&table_id)
        .unwrap();
    let index = cached.inner.index.read().await;
    let remaining_ids: Vec<u64> = index.files.iter().map(|f| f.id.as_u64()).collect();
    assert_eq!(
        remaining_ids,
        vec![1, 3, 4],
        "Files 0 and 2 should be removed, rest preserved"
    );
}

#[tokio::test]
async fn test_purge_compacted_preserves_non_matching_files() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let cache = TableIndexCache::new(
        "test_node".to_string(),
        TableIndexCacheConfig::default(),
    ...
    // Create a table index with files 0, 1, 2
    let db_id = DbId::from(1);
    let table_id = TableId::from(10);
    let table_index_id = TableIndexId::new("test_node", db_id, table_id);
    let index = create_test_index(table_index_id.clone(), 3);
    ...
    // Purge with empty cleanup result — nothing should be removed
    let cleanup = CleanupResult::new();
    let removed = cache
        .purge_compacted_for_tables("test_node", &cleanup, false)
        .await;

    assert_eq!(removed, 0, "Should have removed 0 files");

    // Verify all original files are still there
    let indices = cache.inner.indices.read().await;
    let cached = indices
        .get(&("test_node".to_string(), db_id))
        .unwrap()
        ...
        .unwrap();
    let index = cached.inner.index.read().await;
    assert_eq!(index.files.len(), 3, "All 3 files should remain");
}

#[tokio::test]
async fn test_purge_compacted_with_empty_set_returns_zero() {
    let object_store: Arc<dyn ObjectStore> = Arc::new(InMemory::new());
    let cache = TableIndexCache::new(
        "test_node".to_string(),
        TableIndexCacheConfig::default(),
    ...
    assert_eq!(
        removed, 0,
        "Should have removed 0 files with empty cleanup result"
    );
}
        ...
        for step in test.test_steps {
            match step {
                InitOrUpdateTestStep::SetupPersistSnapshot { seq, databases } => {
                    let snapshot = create_test_persisted_snapshot(test.node_prefix, seq, databases);
                ...
                InitOrUpdateTestStep::SetupPersistTableSnapshot {
                    ...
                    let snapshot = create_test_table_snapshot(table_id.clone(), seq, file_count);
                    let path = TableIndexSnapshotPath::new(
                        table_id.node_id(),
                        table_id.db_id().get(),
                        table_id.table_id().get(),
...
mod purge {
            ...
            for step in test.test_steps {
                match step {
                    PurgeTestStep::SetupPersistIndex {
                        ...
                        let _file_ids =
                            persist_table_index(&self.object_store, &table_id, file_count).await;
                    }
                    ...
                    PurgeTestStep::SetupDatabase { db_id, tables } => {
                        ...
                        for (table_id, file_count) in tables {
                            ...
                            persist_table_index(&self.object_store, &table_index_id, file_count)
                                .await;
                        }
                    }
                    PurgeTestStep::SetupCachedTable {
                        ...
                        let table_index_id = TableIndexId::new(test.node_prefix, db_id, table_id);
    ...
    async fn persist_snapshot(
        ...
        let json = serde_json::to_vec(&snapshot).expect("failed to serialize table index snapshot");
        object_store
