...
pub struct SplitCompact {
    /// Maximum size of the input files that can be compacted
...
impl SplitOrCompact for SplitCompact {
    ...
    fn apply(
        ...
        if !files_to_split.is_empty() {
            ...
            // Not enough budget to compact the minimum set of files (1 start-level file with at most 1 target-level file).
            // The settings are so wrong. Panic for us to look into.
            if files_to_compact.is_empty() {
                panic!("Not enough memory budget to compact the minimum set of files (1 start-level file with at most 1 target-level file). Please adjust compactor settings");
            }
...
mod tests {
    ...
    fn test_compact_too_small_to_compact() {
        ...
        let p_info = Arc::new(PartitionInfoBuilder::new().build());
        let split_compact = SplitCompact::new(MAX_SIZE);
        // Panic because nothing needs to split and limit is too small to compact the minimum set of files
    ...
    fn test_compact_files_no_limit() {
        ...
        insta::assert_yaml_snapshot!(
            format_files_split("files to compact or split:", files_to_compact_or_split.files_to_compact().unwrap() , "files to keep:", &files_to_keep),
            @r###"
        ---
    ...
    fn test_split_files() {
        ...
        // See layout of 2 set of files
        insta::assert_yaml_snapshot!(
            format_files_split("files to compact or split:", &files_to_compact_or_split.files_to_split().unwrap(), "files to keep:", &files_to_keep),
            @r###"
        ---
        - "files to compact or split:"
        - "L0, all files 100b                                                                                  "
