...
func TestDecodeFromString(t *testing.T) {
	...
	if id.String() != "020f755c3c082000" {
		t.Error("expecting string representation to contain the right value")
	}
	if !id.Valid() {
		t.Error("expecting ID to be a valid one")
	}
}
...
func TestDecodeFromAllZeros(t *testing.T) {
	...
	if err == nil {
		t.Error("expecting all zeros ID to not be a valid ID")
	}
}
...
func TestDecodeFromShorterString(t *testing.T) {
	...
	if err == nil {
		t.Error("expecting shorter inputs to error")
	}
	if id.String() != "" {
		t.Error("expecting invalid ID to be serialized into empty string")
	}
}
...
func TestDecodeFromEmptyString(t *testing.T) {
	...
	if err == nil {
		t.Error("expecting empty inputs to error")
	}
	if id.String() != "" {
		t.Error("expecting invalid ID to be serialized into empty string")
	}
...
func TestMarshalling(t *testing.T) {
	...
	if err == nil {
		t.Error("expecting empty ID to not be a valid one")
	}
	...
	if !bytes.Equal(bytes1, bytes2) {
		t.Error("error marshalling/unmarshalling ID")
	}
...
func TestValid(t *testing.T) {
	var id platform.ID
	if id.Valid() {
		t.Error("expecting initial ID to be invalid")
	}

	if platform.InvalidID() != 0 {
		t.Error("expecting invalid ID to return a zero ID, thus invalid")
	}
}
